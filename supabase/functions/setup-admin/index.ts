...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Insert admin_profiles table with full superadmin permissions
    const profileInsertResponse = await fetch(`${SUPABASE_URL}/rest/v1/admin_profiles`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'apikey': SUPABASE_SERVICE_ROLE_KEY,
    ...
    const profileInsertData = await profileInsertResponse.json();
    if (!profileInsertResponse.ok) {
      console.error('Failed to insert admin profile:', profileInsertData);
      throw new Error(profileInsertData.message || 'Failed to create admin profile');
    }
