...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Initialize Navigation Card Links
...
function initializeNavigationCards() {
  ...
  navCards.forEach(card => {
    ...
    if (routeKey && Routes.ADMIN[routeKey]) {
      card.href = Routes.rel(Routes.ADMIN[routeKey]);
      // Prevent default navigation on middle-click/ctrl-click (let browser handle it)
      card.addEventListener('click', (e) => {
        // Only navigate on left-click without modifiers
        if (e.button === 0 && !e.ctrlKey && !e.metaKey && !e.shiftKey) {
          e.preventDefault();
          window.location.href = card.href;
        }
        // Browser handles ctrl/cmd+click, middle-click, shift+click automatically
      });
    }
  });
}
