...
const DB = {
  ...
  async getCurrentProfile() {
    ...
    if (!data) {
      // No database record - create one using JWT metadata role
      // Get role from JWT metadata, default to 'tutor' if not set
      const jwtRole = session.user.user_metadata?.role || 'tutor';
      
      // Define permission flags based on role
      const permissions = {
        'superadmin': {
      ...
      const rolePermissions = permissions[jwtRole] || permissions['tutor'];
      
      const defaultProfile = {
        id: session.user.id,
        email: session.user.email,
        name: session.user.email.split('@')[0],
        role: jwtRole,  // Use JWT metadata role
      ...
      try {
        ...
        console.warn("Failed to auto-insert profile:", err);
      }
      return defaultProfile;
    }
    
    // Database record exists - trust it and return as-is
    return data;
  },
...
async function requirePermission(moduleKey) {
  ...
  if (!allowed) {
    ...
    let redirectPage = Routes.ADMIN.LOGIN;
    const routeMap = {
      'Attendance': Routes.ADMIN.ATTENDANCE,
      'Students': Routes.ADMIN.STUDENTS,
      'Questions': Routes.ADMIN.QUESTIONS,
      'Results': Routes.ADMIN.RESULTS,
      'Sessions': Routes.ADMIN.SESSIONS,
      'Violations': Routes.ADMIN.VIOLATIONS,
      'Settings': Routes.ADMIN.SETTINGS,
      'Logs': Routes.ADMIN.LOGS
    };
    for (const m of modules) {
      if (profile[`allow${m}`]) {
        redirectPage = routeMap[m] || Routes.ADMIN.LOGIN;
        break;
      }
    }
