...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (hasMultipleTutorModules) {
    menuItems.push({ name: 'Dashboard', path: Routes.ADMIN.DASHBOARD, icon: '📊' });
  }

  // Define potential pages and their permission keys
  const permissionPages = [
    { name: 'Attendance', path: Routes.ADMIN.ATTENDANCE, icon: '📋', key: 'allowAttendance' },
    { name: 'Attendance Reports', path: Routes.ADMIN.ATTENDANCE_REPORT, icon: '📈', key: 'allowAttendance' },
    { name: 'Manage Students', path: Routes.ADMIN.STUDENTS, icon: '👥', key: 'allowStudents' },
    { name: 'Question Bank', path: Routes.ADMIN.QUESTIONS, icon: '❓', key: 'allowQuestions' },
    { name: 'View Results', path: Routes.ADMIN.RESULTS, icon: '🏆', key: 'allowResults' },
    { name: 'Active Sessions', path: Routes.ADMIN.SESSIONS, icon: '👁️', key: 'allowSessions' },
    { name: 'Violation Logs', path: Routes.ADMIN.VIOLATIONS, icon: '⚠️', key: 'allowViolations' },
    { name: 'System Settings', path: Routes.ADMIN.SETTINGS, icon: '⚙️', key: 'allowSettings' },
    { name: 'Activity Audit Logs', path: Routes.ADMIN.LOGS, icon: '📜', key: 'allowLogs' }
  ];
  ...
  sidebarEl.innerHTML = `
    ...
    <div class="cbt-sidebar-menu">
      ${menuItems.map(item => `
        <div class="cbt-sidebar-item ${isActive(item.path)}" onclick="window.location.href='${Routes.rel(item.path)}'">
  ...
  // Bind logout action - get button from the newly inserted sidebar
  const logoutBtn = sidebarEl.querySelector('.btn-logout');
  if (logoutBtn) {
